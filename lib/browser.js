...
async function launch(accountId, { headless = false, startUrl = 'about:blank' } = {}) {
  if (!fs.existsSync(CHROME_BIN)) {
    throw new Error('Google Chrome 이 설치되어 있지 않습니다 (/Applications/Google Chrome.app)');
  }
  ...
  const close = async () => {
    ...
    try { execSync(`pkill -f -- "--user-data-dir=${dir}"`, { stdio: 'ignore' }); } catch {}
  };
...
module.exports = { launch, spawnPlain, checkSessionOffline, profileDir, profileExists, hasInstagramSession, PROFILE_ROOT };
